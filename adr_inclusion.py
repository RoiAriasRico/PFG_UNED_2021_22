...
def make_adr(filename):
    ...
    if config.chk_adr == 1:
        print("EXISTE COLUMNA ADR")
    else:
        print("NO EXISTE COLUMNA ADR")
        data['adr'] = None
...
def test_train_adr(filename_adr):
    ...
    #Entrenar propio FastText//OJO!! NO USADO POR PERDIDA DE PRECISION
    ...
    #-----------------------FASTTEXT PESADA CON TFIFDF----------------------------
    #
    #from sklearn.preprocessing import LabelEncoder
    #from gensim.parsing.preprocessing import strip_punctuation

    #codificador = LabelEncoder()
    #y = codificador.fit_transform(df.adr)
    #X = df.text.values
    #lista = []
    #for t in X:
    #    lista.append([i.lemma_ for i in nlp(strip_punctuation(t.lower()))])

    #from sklearn.feature_extraction.text import TfidfVectorizer
    # TF-IDF # se necesita una lista de listas
    #text = []
    #for i in lista:
    #    string = ' '.join(i)
    #    text.append(string)
    ...
    #print(final_tf_idf)
    #from gensim.models import FastText
    #model_ft = FastText(lista, size=50, window=5, min_count=5, iter=10, sorted_vocab=1)
    #tfidf_sent_vectors_ft = []
    #fila = 0
    #errores = 0
    #for sent in lista:  # for each review/sentence
    #    sent_vec = np.zeros(20)
    #    weight_sum = 0;
    #    for palabra in sent:
    #        try:
    #            vec = model_ft.wv[palabra]
    #            tfidf = final_tf_idf[fila, tfidf_feat.index(palabra)]
    #            sent_vec += (vec * tfidf)
    #            weight_sum += tfidf
    #        except:
    #            errores = +1
    ...
    #X_train, X_test, y_train, y_test = train_test_split(X_ft, y, test_size=0.3)
    ...
    #y_pred=mnb_clf.predict(count_test)
    #print("Exactitud del Clasificador de MNB : %2.2f" % accuracy_score(y_test, y_pred))
    #print('La precision del modelo es del {c} % '.format(c=np.round(precision_score(y_test, y_pred, pos_label = 'P'),2)*100))
    #print('El recall del modelo es de {c} % '.format(c=np.round(recall_score(y_test, y_pred, pos_label = 'P'),2)*100))
    #print("el f1 es : " + str(f1_score(y_test, y_pred, pos_label = 'P')*100))
    #y_prob=mnb_clf.predict_proba(count_test)[:,1]
    #print('El clasificador de NB es AUC-ROC : ', roc_auc_score(y_test, y_prob))
    #
    #svc_clf = SVC(probability=True)
    #svc_clf.fit(count_train, y_train)
    #y_pred=svc_clf.predict(count_test)
    #print("Exactitud del Clasificador de SVC : %2.2f" % accuracy_score(y_test, y_pred))
    #print('La precision del modelo es del {c} % '.format(c=np.round(precision_score(y_test, y_pred, pos_label = 'P'),2)*100))
    #print('El recall del modelo es de {c} % '.format(c=np.round(recall_score(y_test, y_pred, pos_label = 'P'),2)*100))
    #print("el f1 es : " + str(f1_score(y_test, y_pred, pos_label = 'P')*100))
    #y_prob=svc_clf.predict_proba(count_test)[:,1]
    #print('El clasificador de svc es AUC-ROC : ', roc_auc_score(y_test, y_prob))
    #
    #knn_clf = KNeighborsClassifier()
    #knn_clf.fit(count_train, y_train)
    #y_pred=knn_clf.predict(count_test)
    #print("Exactitud del Clasificador de KNN : %2.2f" % accuracy_score(y_test, y_pred))
    #print('La precision del modelo es del {c} % '.format(c=np.round(precision_score(y_test, y_pred, pos_label = 'P'),2)*100))
    #print('El recall del modelo es de {c} % '.format(c=np.round(recall_score(y_test, y_pred, pos_label = 'P'),2)*100))
    #print("el f1 es : " + str(f1_score(y_test, y_pred, pos_label = 'P')*100))
    #y_prob=knn_clf.predict_proba(count_test)[:,1]
    #print('El clasificador de knn es AUC-ROC : ', roc_auc_score(y_test, y_prob))
    #
    #rfc_clf = RandomForestClassifier(n_estimators=18)
    #rfc_clf.fit(count_train, y_train)
    #y_pred=rfc_clf.predict(count_test)
    #print("Exactitud del Clasificador de RFC : %2.2f" % accuracy_score(y_test, y_pred))
    #print('La precision del modelo es del {c} % '.format(c=np.round(precision_score(y_test, y_pred, pos_label = 'P'),2)*100))
    #print('El recall del modelo es de {c} % '.format(c=np.round(recall_score(y_test, y_pred, pos_label = 'P'),2)*100))
    #print("el f1 es : " + str(f1_score(y_test, y_pred, pos_label = 'P')*100))
    #y_prob=rfc_clf.predict_proba(count_test)[:,1]
    #print('El clasificador de rfc es AUC-ROC : ', roc_auc_score(y_test, y_prob))
    #----------------------------FIN METRICAS DE CLASIFICADORES------------------------------------
